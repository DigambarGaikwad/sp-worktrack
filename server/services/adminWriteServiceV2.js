...
async function updateRecord(collectionName, id, body) {
  return pocketBaseRequest(`/api/collections/${collectionName}/records/${id}`, { method: "PATCH", body });
}

function makeMap(records, keyFn) {
  const map = new Map();
  records.forEach((r) => {
    const key = keyFn(r);
...
async function syncCollection({ collection, records, keyFn, deactivateMissing = false, deactivateBody = { active: false } }) {
  ...
  for (const record of records) {
    ...
    if (old?.id) {
      await updateRecord(collection, old.id, record);
      counts.updated += 1;
    } else {
  ...
  if (deactivateMissing) {
    for (const [key, old] of existingMap.entries()) {
      if (!incomingKeys.has(key) && old?.id && old.active !== false) {
        await updateRecord(collection, old.id, deactivateBody);
...
function normalizeDepartments(data) {
  ...
  return Array.from(new Set([...fromMainWorks, ...fromEmployees, ...fromCatalog].filter(Boolean)))
    .map((name) => ({ department_code: slug(name), department_name: name, active: true }));
}
...
function normalizeSubworksAndPoints(data) {
  ...
  Object.entries(catalogByType).forEach(([typeCodeRaw, catalog]) => {
    ...
    Object.entries(catalog.subWorks || {}).forEach(([deptNameRaw, list]) => {
      const deptName = clean(deptNameRaw);
      ...
      (Array.isArray(list) ? list : []).forEach((sw, swIndex) => {
        ...
        subworks.push({
          ...
          standard_time: num(sw.standardTime ?? sw.standard_time, 0),
...
async function saveAdminMasterData(rawData = {}) {
  ...
  return {
    ...
    message: deactivateMissing
      ? "Admin master data saved. Missing records marked inactive."
      : "Admin master data saved to PocketBase.",
    results
  };
}
