...
async function getMachineSummary(params = {}) {
  ...
  const [machines, machineTypes, subworks, bookingPoints, lines, bookingStatus, qualityLogs] = await Promise.all([
  ...
  bookingPoints.forEach((bp) => {
    ...
    const key = [clean(bp.machine_type_code), clean(bp.department_code), clean(bp.subwork_code)].join("|");
    bookingStdBySubworkKey.set(key, toNumber(bookingStdBySubworkKey.get(key), 0) + toNumber(bp.standard_time, 0));
  });
  ...
  subworks.forEach((sw) => {
    ...
    if (!typeCode) return;
    const key = [clean(sw.machine_type_code), clean(sw.department_code), clean(sw.subwork_code)].join("|");
    ...
    plannedStdByType.set(typeCode, toNumber(plannedStdByType.get(typeCode), 0) + plannedStd);
  });
  ...
  machineMap.forEach((m) => ensure(m.machineNo, m.machineCategory, m.machineTypeCode));
  ...
  lines.forEach((line) => {
    ...
    item.entryCount += 1;
    if (nature === "rework") item.reworkMinutes += actual;
    else if (nature === "other") item.otherMinutes += actual;
    else { item.normalMinutes += actual; item.completedStandardMinutes += standard; }
  ...
  let items = Array.from(summaryMap.values()).map((x) => {
    ...
    const completionPct = x.plannedStandardMinutes > 0 ? Math.min(100, (x.completedStandardMinutes / x.plannedStandardMinutes) * 100) : 0;
    const efficiencyPct = x.actualMinutes > 0 ? (x.completedStandardMinutes / x.actualMinutes) * 100 : 0;
    return { ...x, standardMinutes: x.plannedStandardMinutes, standardHours: Number((x.plannedStandardMinutes / 60).toFixed(2)), completedStandardHours: Number((x.completedStandardMinutes / 60).toFixed(2)), actualHours: Number((x.actualMinutes / 60).toFixed(2)), remainingHours: Number((x.remainingMinutes / 60).toFixed(2)), overrunHours: Number((x.overrunMinutes / 60).toFixed(2)), completionPct: Number(completionPct.toFixed(1)), efficiencyPct: Number(efficiencyPct.toFixed(1)) };
  });

  if (machineFilter) items = items.filter(x => x.machineNo.toLowerCase().includes(machineFilter.toLowerCase()));
  if (categoryFilter) items = items.filter(x => x.machineCategory.toLowerCase().includes(categoryFilter.toLowerCase()) || x.machineTypeCode.toLowerCase().includes(categoryFilter.toLowerCase()));
...
async function getMachineDetail(params = {}) {
  ...
  const [summaryData, machines, machineTypes, subworks, bookingPoints, qualityPoints, lines, bookingStatus, qualityLogs] = await Promise.all([
    ...
    listAll("production_entry_lines", { perPage: 1000, filter: `machine_no="${machineNo.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`, sort: "-work_date" }),
    listAll("booking_status", { perPage: 1000, filter: `machine_no="${machineNo.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"` }),
    listAll("quality_logs", { perPage: 1000, filter: `machine_no="${machineNo.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`, sort: "-work_date" })
  ]);
  ...
  machineTypes.forEach((t) => { const code = clean(t.type_code || t.id || t.typeCode); const name = clean(t.type_name || t.name || t.typeName); if (code) typeMap.set(code, name || code); });
  const machineCategory = typeMap.get(machineTypeCode) || machineSummary?.machineCategory || machineTypeCode;
  const lineWorkDates = lines.map((line) => clean(line.work_date)).filter(Boolean).sort();
  const firstWorkDate = lineWorkDates[0] || "";
  const lastWorkDate = lineWorkDates[lineWorkDates.length - 1] || "";
  const machineStatus = normalizeMachineStatus(machine.id ? machine : machineSummary || {});
  const machineStartDate = clean(machine.start_date || machine.startDate || firstWorkDate);
  const machineEndDate = machineStatus.toLowerCase() === "completed" ? clean(machine.end_date || machine.completed_date || lastWorkDate) : "";
  const summary = machineSummary || {};
  return { machineNo, machineCategory, machineTypeCode, status: machineStatus, startDate: machineStartDate, endDate: machineEndDate, summary, lines, bookingStatus, qualityLogs };
}

module.exports = { getMachineSummary, getMachineDetail };
