...
async function findOne(collectionName, filter) { const items = await listAll(collectionName, { perPage: 1, filter }); return items[0] || null; }
    ...
    Object.entries(catalog.subWorks || {}).forEach(([deptName, list]) => {
      ...
      (Array.isArray(list) ? list : []).forEach((sw, swIndex) => {
        ...
        (Array.isArray(sw.qualityCheckpoints) ? sw.qualityCheckpoints : []).forEach((qp, i) => {
          ...
          qualityPoints.push({ machine_type_code: clean(typeCode), department_code: deptCode, subwork_code: subworkCode, point_code: slug(pointName), point_name: pointName, input_type: clean(qp.inputType || qp.input_type) === "reading" ? "reading" : "status", mandatory: bool(qp.mandatory, false), sequence_no: i + 1, active: bool(qp.active, true) });
        });
      });
    });
  });
...
async function saveAdminMasterData(rawData = {}) {
  ...
  for (const mt of normalizeMachineTypes(data)) { await upsertByFilter("machine_types", `type_code="${pbEscape(mt.type_code)}"`, mt); counts.machineTypes += 1; }
  for (const m of normalizeMachines(data)) { await upsertByFilter("machines", `machine_no="${pbEscape(m.machine_no)}"`, m); counts.machines += 1; }
  for (const e of normalizeEmployees(data)) { await upsertByFilter("employees", `emp_code="${pbEscape(e.emp_code)}"`, e); counts.employees += 1; }
  for (const s of normalizeShifts(data)) { await upsertByFilter("shifts", `shift_code="${pbEscape(s.shift_code)}"`, s); counts.shifts += 1; }
  for (const d of normalizeDepartments(data)) { await upsertByFilter("departments", `department_code="${pbEscape(d.department_code)}"`, d); counts.departments += 1; }
  const { subworks, bookingPoints, qualityPoints } = normalizeSubworksAndPoints(data);
  for (const sw of subworks) { await upsertByFilter("subworks", [`machine_type_code="${pbEscape(sw.machine_type_code)}"`, `department_code="${pbEscape(sw.department_code)}"`, `subwork_code="${pbEscape(sw.subwork_code)}"`].join(" && "), sw); counts.subworks += 1; }
  for (const bp of bookingPoints) { await upsertByFilter("booking_points", [`machine_type_code="${pbEscape(bp.machine_type_code)}"`, `department_code="${pbEscape(bp.department_code)}"`, `subwork_code="${pbEscape(bp.subwork_code)}"`, `point_code="${pbEscape(bp.point_code)}"`].join(" && "), bp); counts.bookingPoints += 1; }
  for (const qp of qualityPoints) { await upsertByFilter("quality_points", [`machine_type_code="${pbEscape(qp.machine_type_code)}"`, `department_code="${pbEscape(qp.department_code)}"`, `subwork_code="${pbEscape(qp.subwork_code)}"`, `point_code="${pbEscape(qp.point_code)}"`].join(" && "), qp); counts.qualityPoints += 1; }
  for (const reason of normalizeNameList(data.lossReasons)) { await upsertByFilter("loss_reasons", `reason_name="${pbEscape(reason)}"`, { reason_name: reason, active: true }); counts.lossReasons += 1; }
