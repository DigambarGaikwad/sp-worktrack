// renderer/capacity/capacityV4.js
// Capacity Planning V4: monthly manpower balance, department override, dependencies and skill-history learning.

(function () {
  const API = window.SPWT_CONFIG?.API_BASE_URL || window.location.origin || "http://localhost:3032";
  const DEFAULT_MIN = 465;
  const STORAGE_KEY = "spwt_capacity_plan_v4";
  const $ = (id) => document.getElementById(id);

  const state = {
    master: {}, absences: [], skills: [], progress: [], monthProgress: [],
    demand: [], targets: [], dependencies: [], shiftPlans: [], overtimeEmpIds: [], deptOverrides: {},
  ...
  function workingDatesBetween(fromValue, toValue) {
    ...
    const dates = [];
    for (let d = new Date(from); d <= to; d.setDate(d.getDate() + 1)) if (d.getDay() !== 0) dates.push(dateKey(d));
    return dates;
  }
  ...
  function allDepartments() {
    const set = new Set();
    machineTypes().forEach((t) => (workCatalog(t.id).mainWorks || []).forEach((d) => set.add(clean(d))));
    state.skills.forEach((s) => { const d = clean(s.skill_department_name || s.department_name); if (d) set.add(d); });
  ...
  function allRegularSelected(exceptShiftId = "") { const set = new Set(); state.shiftPlans.forEach((sp) => { if (sp.id !== exceptShiftId) (sp.employeeIds || []).forEach((id) => set.add(key(id))); }); return set; }
  ...
  function toggleShiftEmployee(shiftId, empId, checked) { state.shiftPlans.forEach((sp) => { sp.employeeIds = (sp.employeeIds || []).filter((id) => key(id) !== key(empId)); }); if (checked) { const sp = state.shiftPlans.find((x) => x.id === shiftId); if (sp) sp.employeeIds = [...(sp.employeeIds || []), empId]; } state.plan = null; renderAll(); }
  ...
  function employeeBaseDept(emp) {
    const override = clean(state.deptOverrides[emp.empId]);
    if (override) return override;
    const scores = [...empDeptScores(emp.empId).entries()].sort((a, b) => b[1] - a[1]);
    return clean(scores[0]?.[0] || emp.department || "Unassigned");
  }
  function manpowerByDept() {
    const map = new Map();
    activeEmployees().forEach((emp) => {
      const dept = employeeBaseDept(emp);
      const old = map.get(dept) || { dept, count: 0, employees: [] };
      old.count += 1; old.employees.push(emp); map.set(dept, old);
    });
    return map;
  }
  function hasSomeDeptHistory(empId, dept) { return [...empDeptScores(empId).entries()].some(([d, score]) => key(d) === key(dept) && score > 0); }
  ...
  function renderTargets() {
    ...
    const rows = state.targets.map((t) => {
      ...
      return `<tr><td>${esc(t.typeName)}</td><td>${esc(t.qty)}</td><td>${esc(st.completedQty)} (${st.eqQty.toFixed(1)} eq)</td><td>${fmtHr(st.stdPerMachine)}</td><td>${fmtHr(targetStd)}</td><td>${fmtHr(expectedStd)}</td><td>${fmtHr(st.doneStd)}</td><td><span class="badge ${behind ? "noskill" : stopOt ? "complete" : "medium"}">${esc(status)}</span></td><td>${behind ? fmtHr(Math.abs(gap)) : "0.0 h"}</td><td><button class="remove-btn" data-remove-target="${esc(t.id)}" type="button">Remove</button></td></tr>`;
    }).join("");
    host.innerHTML = `<table class="cap-table"><thead><tr><th>Machine Type</th><th>Month Qty</th><th>Completed Qty</th><th>Std Hrs / Machine</th><th>Target Std Hrs</th><th>Expected Till Today</th><th>Done Std Hrs</th><th>Status</th><th>OT Need</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table>`;
    document.querySelectorAll("[data-remove-target]").forEach((el) => el.onclick = () => removeTarget(el.dataset.removeTarget));
    renderTargetSummary();
  }

  function targetDeptRows() {
    const map = new Map();
    const month = $("targetMonth").value || currentMonth();
    const remainingDays = Math.max(1, workingDatesBetween(today(), monthEnd(month)).length);
    const manpower = manpowerByDept();
    ...
    return [...map.values()].map((x) => {
      ...
      const need = balance > 0 ? Math.ceil(balance / (remainingDays * DEFAULT_MIN)) : 0;
      const current = manpower.get(x.dept)?.count || 0;
      return { ...x, balance, need, current, gap: current - need };
    }).sort((a, b) => b.target - a.target);
  }

  function trainingSuggestions(rows) {
    const shortage = rows.filter((r) => r.gap < 0);
    const surplus = rows.filter((r) => r.gap > 0);
    const employees = activeEmployees();
    return shortage.map((r) => {
      const candidates = surplus.flatMap((s) => (manpowerByDept().get(s.dept)?.employees || []))
        .filter((emp, idx, arr) => arr.findIndex((x) => key(x.empId) === key(emp.empId)) === idx)
        .filter((emp) => hasSomeDeptHistory(emp.empId, r.dept))
        .slice(0, 5);
      const names = candidates.map((e) => `${e.empId}-${e.name}`).join(", ");
      return { dept: r.dept, need: Math.abs(r.gap), names: names || "No cross-skill history found" };
    });
  }

  function renderTargetSummary() {
    const host = $("targetSummaryHost");
    const rows = targetDeptRows();
    if (!rows.length) { host.innerHTML = ""; return; }
    const totalHave = activeEmployees().length;
    const totalNeed = rows.reduce((s, r) => s + r.need, 0);
    const shortageCount = rows.reduce((s, r) => s + Math.max(0, -r.gap), 0);
    const surplusCount = rows.reduce((s, r) => s + Math.max(0, r.gap), 0);
    const cards = `<div class="target-manpower-cards">${[
      ["Total Manpower", totalHave, "green"],
      ["Manpower Need", totalNeed, totalNeed <= totalHave ? "green" : "red"],
      ["Dept Shortage", shortageCount, shortageCount ? "red" : "green"],
      ["Dept Surplus", surplusCount, surplusCount ? "orange" : ""]
    ].map(([l, v, t]) => `<div class="metric-card"><div class="metric-label">${l}</div><div class="metric-value ${t}">${v}</div></div>`).join("")}</div>`;

    const rowHtml = rows.map((x) => {
      const people = (manpowerByDept().get(x.dept)?.employees || []).map((e) => `<button class="dept-person" data-edit-dept="${esc(e.empId)}">${esc(e.empId)} - ${esc(e.name)} (${esc(employeeBaseDept(e))})</button>`).join(" ") || "-";
      const gapText = x.gap >= 0 ? `Surplus ${x.gap}` : `Short ${Math.abs(x.gap)}`;
      return `<tr><td>${esc(x.dept)}</td><td>${fmtHr(x.target)}</td><td>${fmtHr(x.done)}</td><td>${fmtHr(x.balance)}</td><td>${esc(x.current)}</td><td>${esc(x.need)}</td><td><span class="badge ${x.gap < 0 ? "noskill" : x.gap > 0 ? "complete" : "medium"}">${esc(gapText)}</span></td><td>${people}</td></tr>`;
    }).join("");

    const trainRows = trainingSuggestions(rows).map((x) => `<tr><td>${esc(x.dept)}</td><td>${esc(x.need)}</td><td>${esc(x.names)}</td></tr>`).join("");
    const train = trainRows ? `<div class="target-summary-title">Training Need / Cross-skill Recommendation</div><table class="cap-table"><thead><tr><th>Short Department</th><th>People Need</th><th>Recommended Names with Some History</th></tr></thead><tbody>${trainRows}</tbody></table>` : `<div class="complete-note">No department-wise manpower shortage found in monthly target.</div>`;

    host.innerHTML = `${cards}<div class="target-summary-title">Department-wise Monthly Manpower Need</div><table class="cap-table"><thead><tr><th>Department</th><th>Target Std Hrs</th><th>Done Std Hrs</th><th>Balance Std Hrs</th><th>Current Manpower</th><th>Need</th><th>Gap</th><th>People / Department Override</th></tr></thead><tbody>${rowHtml}</tbody></table>${train}`;
    document.querySelectorAll("[data-edit-dept]").forEach((el) => el.onclick = () => openDeptModal(el.dataset.editDept));
  }

  function openDeptModal(empId) {
    const emp = activeEmployees().find((e) => key(e.empId) === key(empId));
    if (!emp) return;
    const depts = allDepartments();
    const current = employeeBaseDept(emp);
    let modal = $("deptOverrideModal");
    if (!modal) {
      modal = document.createElement("div");
      modal.id = "deptOverrideModal";
      modal.className = "cap-modal";
      document.body.appendChild(modal);
    }
    modal.innerHTML = `<div class="cap-modal-box"><h3>Change Employee Department</h3><p>${esc(emp.empId)} - ${esc(emp.name)}</p><label>Department<select id="deptOverrideSelect">${depts.map((d) => `<option value="${esc(d)}" ${key(d) === key(current) ? "selected" : ""}>${esc(d)}</option>`).join("")}</select></label><div class="modal-actions"><button id="deptSaveBtn" type="button">Save</button><button id="deptClearBtn" type="button">Clear Override</button><button id="deptCancelBtn" type="button">Cancel</button></div></div>`;
    modal.classList.add("show");
    $("deptSaveBtn").onclick = () => { state.deptOverrides[emp.empId] = $("deptOverrideSelect").value; modal.classList.remove("show"); renderAll(); };
    $("deptClearBtn").onclick = () => { delete state.deptOverrides[emp.empId]; modal.classList.remove("show"); renderAll(); };
    $("deptCancelBtn").onclick = () => modal.classList.remove("show");
  }
  ...
  function addDependency() { const typeId = $("depTypeSelect").value, typeNameText = typeName(typeId), work = $("depWorkSelect").value, subwork = $("depSubworkSelect").value, dependsWork = $("depOnWorkSelect").value, dependsSubwork = $("depOnSubworkSelect").value; if (!typeId || !work || !subwork || !dependsWork || !dependsSubwork) return; const dup = state.dependencies.some((d) => norm(d.typeId) === norm(typeId) && key(d.work) === key(work) && key(d.subwork) === key(subwork) && key(d.dependsWork) === key(dependsWork) && key(d.dependsSubwork) === key(dependsSubwork)); if (!dup) state.dependencies.push({ id: makeId(), typeId, typeName: typeNameText, work, subwork, dependsWork, dependsSubwork }); state.plan = null; renderAll(); }
  function removeDependency(id) { state.dependencies = state.dependencies.filter((d) => d.id !== id); state.plan = null; renderAll(); }
  function renderDependencies() { renderDependencySelectors(); const host = $("dependencyTable"); if (!state.dependencies.length) { host.innerHTML = `<div class="empty-state">No work dependencies saved yet. Add rules only where one work cannot start before another work is complete.</div>`; return; } const rows = state.dependencies.map((d) => `<tr><td>${esc(d.typeName)}</td><td><span class="dependency-chip">${esc(d.work)} / ${esc(d.subwork)}</span></td><td>after</td><td><span class="dependency-chip">${esc(d.dependsWork)} / ${esc(d.dependsSubwork)}</span></td><td><button class="remove-btn" data-remove-dependency="${esc(d.id)}" type="button">Remove</button></td></tr>`).join(""); host.innerHTML = `<table class="cap-table"><thead><tr><th>Machine Type</th><th>Work</th><th>Rule</th><th>Depends On</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table>`; document.querySelectorAll("[data-remove-dependency]").forEach((el) => el.onclick = () => removeDependency(el.dataset.removeDependency)); }
  ...
  function depsSatisfied(task, tasks) { if ($("sequenceMode")?.value === "free") return true; return depRulesFor(task).every((r) => { const depTask = tasks.find((t) => t.demandId === task.demandId && key(t.dept) === key(r.dependsWork) && key(t.subwork) === key(r.dependsSubwork)); return !depTask || depTask.remainingStd <= 0.01; }); }
  ...
  function bestEmployeeForTask(empStates, skillMap, task) { let best = null; empStates.filter((e) => e.available > 0.5).forEach((e) => { const skill = skillFor(skillMap, e.emp, task), conf = confidenceScore(skill); const deptMatch = key(employeeBaseDept(e.emp)) === key(task.dept) ? 1 : 0; const score = (conf * 10000) + historyScore(skill) + (deptMatch * 500) + Math.min(e.available, 999) / 1000; if (!best || score > best.score || (score === best.score && clean(e.emp.name).localeCompare(clean(best.state.emp.name)) < 0)) best = { state: e, skill, score }; }); return best; }
  function assignTasks(date, empStates, tasks, skillMap, byDemand, completedDemand, assignments, isOvertime = false) { while (empStates.some((e) => e.available > 0.5)) { const task = nextOpenTask(tasks); if (!task) break; const picked = bestEmployeeForTask(empStates, skillMap, task); if (!picked) break; const eff = planningEff(picked.skill), possibleStd = picked.state.available * (eff / 100), assignStd = Math.min(task.remainingStd, possibleStd), planMin = assignStd * (100 / eff); if (assignStd <= 0.01 || planMin <= 0.01) break; task.remainingStd -= assignStd; picked.state.available -= planMin; const demandItems = byDemand.get(task.demandId) || []; const machineComplete = !completedDemand.has(task.demandId) && demandItems.length && demandItems.every((t) => t.remainingStd <= 0.01); if (machineComplete) completedDemand.add(task.demandId); assignments.push({ date, empId: picked.state.emp.empId, empName: picked.state.emp.name, empDept: employeeBaseDept(picked.state.emp), shiftName: picked.state.shiftName, overtime: isOvertime, machineNo: task.machineNo, typeName: task.typeName, work: task.dept, subwork: task.subwork, priority: task.priority, stdMinutes: assignStd, planMinutes: planMin, balanceMinutes: picked.state.available, confidence: confidenceText(picked.skill), historyCount: num(picked.skill?.history_count, 0), planningEff: eff, targetDate: task.targetDate, machineComplete }); } }
  ...
  function renderResult() { const c = summarizeCapacity(), tasks = buildTasks(), originalReq = tasks.reduce((s, t) => s + t.stdMinutes, 0), completedBefore = tasks.reduce((s, t) => s + num(t.completedBeforeStd, 0), 0), req = tasks.reduce((s, t) => s + Math.max(0, t.remainingStd), 0), remaining = state.plan ? state.plan.remainingStd : req, gap = c.regularMin - req, withOtGap = (c.regularMin + c.otMin) - req; $("resultMetrics").innerHTML = [metric("Machine Demand", state.demand.length), metric("Original Std Hrs", fmtHr(originalReq)), metric("Done From Entries", fmtHr(completedBefore), completedBefore ? "green" : ""), metric("Remaining Std Hrs", fmtHr(req)), metric("Regular Gap", fmtHr(gap), gap >= 0 ? "green" : "red"), metric("Gap With OT", fmtHr(withOtGap), withOtGap >= 0 ? "green" : "red"), metric("Planned OT", state.plan ? fmtHr(state.plan.overtimeMinutes) : "-", state.plan?.overtimeMinutes ? "orange" : ""), metric("Blocked Work", state.plan ? state.plan.blockedNotes.length : "-", state.plan?.blockedNotes?.length ? "orange" : ""), metric("Plan Completed", state.plan ? `${state.plan.doneMachines}/${state.plan.totalMachines}` : "-"), metric("Unplanned Std Hrs", state.plan ? fmtHr(remaining) : "-")].join(""); const box = $("warningBox"); box.className = `warning-box ${withOtGap >= 0 ? "ok" : ""}`; if (!state.demand.length) box.textContent = "Add machine demand to see shortage, overtime and multiskilling requirement."; else if (gap >= 0) box.textContent = "Regular selected shift capacity is sufficient. Overtime can be stopped unless target date risk exists."; else if (withOtGap >= 0) box.textContent = `Regular capacity is short by ${fmtHr(Math.abs(gap))}, but selected OT preference can cover it.`; else box.textContent = `Even with selected OT, shortage remains ${fmtHr(Math.abs(withOtGap))}. Need more manpower, multiskilling, target date change, or reduce demand.`; }

  function rowPlanHtml(a) { const cls = key(a.confidence).replace(/\s+/g, "") || "noskill", status = a.machineComplete ? "Machine completed after this work" : a.overtime ? `OT | ${a.confidence}` : `${a.confidence} | history ${a.historyCount}`; const statusClass = a.machineComplete ? "complete" : a.overtime ? "ot" : cls === "noskill" ? "noskill" : cls; return `<tr><td>${esc(a.shiftName)}</td><td>${esc(a.priority)}</td><td>${esc(a.machineNo)}</td><td>${esc(a.typeName)}</td><td>${esc(a.work)}</td><td>${esc(a.subwork)}</td><td>${fmtMin(a.stdMinutes)}</td><td>${fmtMin(a.planMinutes)}</td><td>${fmtMin(a.balanceMinutes)}</td><td><span class="badge ${statusClass}">${esc(status)}</span></td></tr>`; }
  function renderPlan() { const host = $("employeePlanHost"); if (!state.plan) { host.innerHTML = `<div class="empty-state">Production plan not generated yet.</div>`; return; } if (!state.plan.assignments.length && !state.plan.completionNotes.length) { host.innerHTML = `<div class="empty-state">No assignment generated. Check demand, date range, shift manpower, dependency rules and skill matrix.</div>`; return; } const byDate = new Map(); state.plan.dates.forEach((date) => byDate.set(date, { rows: [], notes: [] })); state.plan.assignments.forEach((a) => { if (!byDate.has(a.date)) byDate.set(a.date, { rows: [], notes: [] }); byDate.get(a.date).rows.push(a); }); state.plan.completionNotes.forEach((n) => byDate.get(n.date)?.notes.push(n)); host.innerHTML = [...byDate.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([date, bucket]) => { const rows = bucket.rows.sort((a, b) => clean(a.empName).localeCompare(clean(b.empName)) || a.priority - b.priority || clean(a.machineNo).localeCompare(clean(b.machineNo))); const byEmp = new Map(); rows.forEach((r) => { if (!byEmp.has(r.empId)) byEmp.set(r.empId, []); byEmp.get(r.empId).push(r); }); const notes = bucket.notes.map((n) => `<div class="complete-note">P${esc(n.priority)} | ${esc(n.machineNo)} | ${esc(n.typeName)} — ${esc(n.message)}</div>`).join(""); const empHtml = [...byEmp.entries()].map(([empId, empRows]) => { const first = empRows[0], totalPlan = empRows.reduce((s, x) => s + x.planMinutes, 0); return `<div class="employee-card"><div class="employee-head"><div><div class="employee-name">${esc(empId)} - ${esc(first.empName)}</div><div class="employee-meta">${esc(first.empDept || "No department")}</div></div><div class="employee-meta">Day Load: ${fmtHr(totalPlan)}</div></div><div class="table-host"><table class="cap-table"><thead><tr><th>Shift</th><th>Priority</th><th>Machine No.</th><th>Type</th><th>Work</th><th>Subwork</th><th>Std Min</th><th>Plan Min</th><th>Balance</th><th>Status</th></tr></thead><tbody>${empRows.map(rowPlanHtml).join("")}</tbody></table></div></div>`; }).join(""); const blocked = (state.plan.blockedNotes || []).filter((n) => true).map((n) => `<div class="blocked-note">P${esc(n.priority)} | ${esc(n.machineNo)} | ${esc(n.work)} / ${esc(n.subwork)} blocked by ${esc(n.reason)}</div>`).join(""); return `<div class="date-plan-card"><div class="date-title">${dateText(date)} — ${fmtHr(rows.reduce((s, x) => s + x.planMinutes, 0))}</div>${notes}${empHtml || `<div class="empty-state">No work assigned on this date.</div>`}${blocked}</div>`; }).join(""); }
  function planText() { if (!state.plan?.assignments?.length && !state.plan?.completionNotes?.length) return "No production plan generated."; const lines = ["SP WorkTrack - Date-wise Employee Production Plan", `Period: ${dateText($("fromDate").value)} to ${dateText($("toDate").value)}`, `Progress considered up to: ${dateText(progressCutoffDate())}`, ""]; const byDate = new Map(); (state.plan.dates || []).forEach((d) => byDate.set(d, { rows: [], notes: [] })); state.plan.assignments.forEach((a) => byDate.get(a.date)?.rows.push(a)); state.plan.completionNotes.forEach((n) => byDate.get(n.date)?.notes.push(n)); byDate.forEach((bucket, date) => { lines.push(dateText(date)); bucket.notes.forEach((n) => lines.push(`  P${n.priority} | ${n.machineNo} | ${n.typeName} | ${n.message}`)); const byEmp = new Map(); bucket.rows.forEach((a) => { if (!byEmp.has(a.empId)) byEmp.set(a.empId, []); byEmp.get(a.empId).push(a); }); byEmp.forEach((rows, empId) => { lines.push(`  ${empId} - ${rows[0].empName}`); rows.sort((a, b) => a.priority - b.priority || clean(a.machineNo).localeCompare(clean(b.machineNo))).forEach((r) => lines.push(`    ${r.shiftName} | P${r.priority} | ${r.machineNo} | ${r.work} | ${r.subwork} | Std ${fmtMin(r.stdMinutes)} | Plan ${fmtMin(r.planMinutes)} | ${r.overtime ? "OT" : r.confidence}`)); }); lines.push(""); }); return lines.join("\n"); }

  function savePlan() { const data = { version: 4, savedAt: new Date().toISOString(), fromDate: $("fromDate").value, toDate: $("toDate").value, targetMonth: $("targetMonth").value, otMaxMinutes: $("otMaxMinutes").value, sequenceMode: $("sequenceMode").value, seq: state.seq, shiftSeq: state.shiftSeq, demand: state.demand, targets: state.targets, dependencies: state.dependencies, shiftPlans: state.shiftPlans, overtimeEmpIds: state.overtimeEmpIds, deptOverrides: state.deptOverrides }; localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); alert("Capacity plan saved with targets, shifts, OT, dependencies and department overrides."); }
  function restoreSavedPlanOnce() { if (state.restored) return; state.restored = true; try { const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; const data = JSON.parse(raw); if (data?.fromDate) $("fromDate").value = data.fromDate; if (data?.toDate) $("toDate").value = data.toDate; if (data?.targetMonth) $("targetMonth").value = data.targetMonth; if (data?.otMaxMinutes != null) $("otMaxMinutes").value = data.otMaxMinutes; if (data?.sequenceMode && $("sequenceMode")) $("sequenceMode").value = data.sequenceMode; state.demand = Array.isArray(data?.demand) ? data.demand : []; state.targets = Array.isArray(data?.targets) ? data.targets : []; state.dependencies = Array.isArray(data?.dependencies) ? data.dependencies : []; state.shiftPlans = Array.isArray(data?.shiftPlans) ? data.shiftPlans : []; state.overtimeEmpIds = Array.isArray(data?.overtimeEmpIds) ? data.overtimeEmpIds : []; state.deptOverrides = data?.deptOverrides || {}; state.seq = Math.max(num(data?.seq, 1), ...state.demand.map((d) => num(d.seq, 0) + 1), 1); state.shiftSeq = Math.max(num(data?.shiftSeq, 1), state.shiftPlans.length + 1, 1); } catch (err) { console.warn("Saved capacity plan restore failed", err); } }
  ...
  function wire() {
    $("fromDate").value = today(0); $("toDate").value = today(6); if ($("targetMonth")) $("targetMonth").value = currentMonth();
    ...
    ["fromDate", "targetMonth"].forEach((id) => $(id).onchange = loadData); ["toDate", "otMaxMinutes", "sequenceMode"].forEach((id) => $(id).onchange = recalc); ["depTypeSelect", "depWorkSelect", "depOnWorkSelect"].forEach((id) => $(id).onchange = () => { renderDependencySelectors(); });
  }
