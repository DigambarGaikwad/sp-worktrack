...
// Unifies dashboard button styling and replaces observation prompt with inline field + save button.
...
(function () {
  ...
  function addButtonStyles() {
    ...
    style.textContent = `
      ...
      .qr-observation-textarea:focus {
        border-color: #0b3f73;
        box-shadow: 0 0 0 3px rgba(11,63,115,.12);
      }
      .qr-observation-actions {
        display: flex;
        align-items: center;
        gap: 8px;
        margin-top: 8px;
  ...
  function ensureObservationBox() {
    ...
    saveBtn?.addEventListener("click", () => {
      lastSavedObservation = clean(textarea.value);
      window.__SPWT_QUALITY_REPORT_OBSERVATION = lastSavedObservation;
      if (status) status.textContent = lastSavedObservation ? "Observation saved for report." : "Observation cleared.";
      setTimeout(() => { if (status) status.textContent = ""; }, 2500);
    });

    clearBtn?.addEventListener("click", () => {
      textarea.value = "";
      lastSavedObservation = "";
      window.__SPWT_QUALITY_REPORT_OBSERVATION = "";
      if (status) status.textContent = "Observation cleared.";
      setTimeout(() => { if (status) status.textContent = ""; }, 2500);
      textarea.focus();
    });
  }
  ...
  function openObservationBox() {
    ...
    textarea.value = window.__SPWT_QUALITY_REPORT_OBSERVATION || lastSavedObservation || textarea.value || "";
