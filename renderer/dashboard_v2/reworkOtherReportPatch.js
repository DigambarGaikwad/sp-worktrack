...
(function () {
  ...
  function tableRows(rows) { return rows.length ? rows.map(r => `<tr><td>${esc(r.workDate)}</td><td>${esc(r.machine)}</td><td>${esc(r.empName)}<br><span class="muted">${esc(r.empCode)}</span></td><td>${esc(r.department)}</td><td>${esc(r.subWork)}</td><td>${esc(r.rootArea || "-")}</td><td>${esc(r.reason || "-")}</td><td>${esc(r.remark || "-")}</td><td style="text-align:right;font-weight:900;">${esc(r.actualHours)}</td></tr>`).join("") : `<tr><td colspan="9" class="empty">No records found.</td></tr>`; }
